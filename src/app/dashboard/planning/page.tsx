...
const VERDICT_STYLES: Record<MissionAssessmentResult["verdict"], { bg: string; border: string; text: string; icon: string }> = {
  ...
  "CONDITIONAL": { bg: "rgba(230,201,116,0.1)", border: "rgba(230,201,116,0.4)", text: "#e6c974", icon: "⚠" },
};
...
function PlanningForm({ onSubmit, isLoading }: {
  ...
  return (
    <form
      onSubmit={handle}
      className="rounded-xl border border-[#605943] bg-[#24231f] p-5 space-y-5"
    >
      <h2 className="text-base font-semibold text-[#e8e7e5]">Mission Parameters</h2>

      {/* Name + type */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs text-[#96938d] mb-1.5">Mission Name</label>
          <input
            required
            value={form.missionName}
            onChange={(e) => setForm((f) => ({ ...f, missionName: e.target.value }))}
            placeholder="e.g. ISS EVA-89 prep check"
            className="w-full rounded-lg bg-[#29271f] border border-[#3a3830] text-[#e8e7e5] text-sm px-3 py-2 placeholder-[#605943] focus:outline-none focus:border-[#e6c974] transition-colors"
          />
        </div>
        <div>
          <label className="block text-xs text-[#96938d] mb-1.5">Mission Type</label>
          <select
            value={form.missionType}
            onChange={(e) => setForm((f) => ({ ...f, missionType: e.target.value as MissionParameters["missionType"] }))}
            className="w-full rounded-lg bg-[#29271f] border border-[#3a3830] text-[#e8e7e5] text-sm px-3 py-2 focus:outline-none focus:border-[#e6c974] transition-colors"
          >
            {MISSION_TYPES.map((t) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs text-[#96938d] mb-1.5">Planned Start (UTC)</label>
          <input
            required
            type="datetime-local"
        ...
        <div>
          <label className="block text-xs text-[#96938d] mb-1.5">
            Planned End (UTC)
            {durationHours > 0 && (
              <span className="ml-2 text-[#e6c974]">({durationHours.toFixed(1)}h)</span>
            )}
          </label>
          <input
            required
      ...
      {/* Activities */}
      <div>
        <label className="block text-xs text-[#96938d] mb-2">Planned Activities</label>
          ...
          {AVAILABLE_ACTIVITIES.map((act) => {
            ...
            return (
              <button
                ...
                style={{
                  backgroundColor: selected ? "#e6c97422" : "#29271f",
                  border:          `1px solid ${selected ? "#e6c974" : "#3a3830"}`,
                  color:           selected ? "#e6c974"   : "#96938d",
                }}
              >
                {act.replace(/_/g, " ")}
              </button>
      ...
      {/* Notes */}
      <div>
        <label className="block text-xs text-[#96938d] mb-1.5">Additional Notes (optional)</label>
        <textarea
          rows={2}
          value={form.notes}
          onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
          placeholder="Specific constraints, crew details, equipment requirements…"
          className="w-full rounded-lg bg-[#29271f] border border-[#3a3830] text-[#e8e7e5] text-sm px-3 py-2 placeholder-[#605943] focus:outline-none focus:border-[#e6c974] transition-colors resize-none"
        />
      </div>
      ...
      <button
        ...
        className="w-full py-2.5 rounded-lg text-sm font-semibold bg-[#e6c974] text-[#100f0e] hover:bg-[#c3ac6a] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {isLoading ? (
          <>
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm text-[#e8e7e5] mt-1 leading-relaxed">{result.verdictReason}</p>
          <p className="text-xs text-[#96938d] mt-1">
            Overall risk score:{" "}
            <span
              className="font-bold"
              style={{ color: result.overallRiskScore >= 7 ? "#f87171" : result.overallRiskScore >= 4 ? "#e6c974" : "#4ade80" }}
            >
      ...
      <div className="rounded-xl border border-[#605943] bg-[#24231f] p-4">
        <h3 className="text-sm font-semibold text-[#e8e7e5] mb-3">Risk Factors</h3>
              ...
              return (
                <details key={rf.id} className="group rounded-lg border" style={{ borderColor: rc + "33" }}>
                  ...
                  >
                    ...
                    <span className="text-xs font-semibold text-[#e8e7e5] flex-1">{rf.title}</span>
                    <span
                    ...
                    <svg className="w-3 h-3 text-[#605943] group-open:rotate-180 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path d="M19 9l-7 7-7-7" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </summary>
                  <div className="px-3 pb-3 pt-2 space-y-1.5">
                    <p className="text-xs text-[#96938d] leading-relaxed">{rf.description}</p>
                    {rf.mitigationSteps.length > 0 && (
                      <div>
                        <p className="text-[10px] font-semibold text-[#605943] uppercase tracking-widest mb-1">Mitigation</p>
                        <ul className="space-y-0.5">
                          {rf.mitigationSteps.map((step, i) => (
                            <li key={i} className="text-xs text-[#96938d] flex gap-1.5">
                              <span className="text-[#e6c974] flex-shrink-0">·</span>{step}
                            </li>
                          ))}
                        </ul>
                      </div>
      ...
      {/* Activity guidance */}
      <div className="rounded-xl border border-[#605943] bg-[#24231f] p-4">
        <h3 className="text-sm font-semibold text-[#e8e7e5] mb-3">Activity Guidance</h3>
            ...
            return (
              <div key={i} className="rounded-lg border p-3" style={{ borderColor: rc + "33", backgroundColor: rc + "06" }}>
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <p className="text-xs font-semibold text-[#e8e7e5]">
                ...
                <p className="text-xs text-[#96938d] leading-relaxed mb-1.5">{ag.recommendation}</p>
                {ag.precautions.length > 0 && (
                  <ul className="space-y-0.5">
                    {ag.precautions.map((p, j) => (
                      <li key={j} className="text-[10px] text-[#605943] flex gap-1.5">
                        <span className="text-[#8369ce]">›</span>{p}
                      </li>
                    ))}
                  </ul>
                )}
      ...
      {result.alternativeWindows.length > 0 && (
        <div className="rounded-xl border border-[#605943] bg-[#24231f] p-4">
          <h3 className="text-sm font-semibold text-[#e8e7e5] mb-3">Alternative Windows</h3>
          <div className="space-y-2">
            {result.alternativeWindows.map((aw: AlternativeWindow, i: number) => {
              const sc = aw.improvementScore >= 8 ? "#4ade80" : aw.improvementScore >= 5 ? "#e6c974" : "#96938d";
              return (
                <div key={i} className="flex items-start gap-3 rounded-lg bg-[#29271f] px-3 py-2.5">
                  <div
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[#96938d] mt-0.5">{aw.reasonWhy}</p>
                  </div>
                </div>
              );
            })}
...
export default function PlanningPage() {
  ...
  return (
    <div className="min-h-screen bg-[#100f0e] px-4 py-6 md:px-8 max-w-4xl mx-auto">
      {/* Header */}
      <div className="mb-6">
      ...
      <div className="space-y-5">
        ...
        {mutation.isPending && (
          <div className="rounded-xl border border-[#3a3830] bg-[#24231f] p-8 flex flex-col items-center gap-3">
            <div className="relative w-10 h-10">
              <span className="absolute inset-0 rounded-full border-2 border-[#8369ce] border-t-transparent animate-spin" />
            </div>
            <p className="text-sm text-[#96938d]">IBM Granite is analysing conditions…</p>
            <p className="text-xs text-[#605943]">Fetching live telemetry · running risk model · generating recommendations</p>
          </div>
        )}
